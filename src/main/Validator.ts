...
export class Validator<T> {
  ...
  public static hexStringValidationRule<T>(propertyName: keyof T, optional?: boolean) {
    const hexRegExp = new RegExp('0x[0-9a-fA-F]*');

    return Validator.validationRule<T>(propertyName, {
      check: (val) =>
        typeof val === 'string' && hexRegExp.test(val),
      typecast: (val) => {
        const stringVal = String(val);

        // @TODO check if this funciton needs more checks
        return `0x${stringVal}`;
      },
      optional,
    });
